...
export function IntegratedNarrationAssistant({ 
  ...
  const loadUserSettings = async () => {
    try {
      ...
      if (profilesResponse.success && profilesResponse.data?.length > 0) {
        const defaultProfile = profilesResponse.data.find(p => p.is_default) || profilesResponse.data[0];
        if (defaultProfile) {
    ...
    if (update.job_id === narrationState.jobId) {
      ...
      setSlides(prevSlides => 
        prevSlides.map((slide, index) => ({
          ...slide,
          status: index < update.current_slide ? 'completed' : 
                  index === update.current_slide ? 'generating' : 'pending'
        }))
      );
    ...
    try {
      ...
      const mockSlides: SlideNarration[] = [
        { 
          slideId: 'slide1', 
          title: 'Introduction', 
          content: 'Welcome to this presentation about our innovative solution that transforms how teams collaborate.', 
          status: 'pending' 
        },
        { 
          slideId: 'slide2', 
          title: 'Problem Statement', 
          content: 'Today\'s teams face challenges with communication, project management, and maintaining productivity across different time zones.', 
          status: 'pending' 
        },
        { 
          slideId: 'slide3', 
          title: 'Our Solution', 
          content: 'We introduce a comprehensive platform that integrates real-time collaboration, intelligent task management, and seamless workflow automation.', 
          status: 'pending' 
        },
        { 
          slideId: 'slide4', 
          title: 'Key Features', 
          content: 'Our solution includes AI-powered insights, automated reporting, cross-platform compatibility, and enterprise-grade security.', 
          status: 'pending' 
        },
        { 
          slideId: 'slide5', 
          title: 'Results & Impact', 
          content: 'Teams using our platform report 40% improvement in productivity, 60% faster project completion, and 90% user satisfaction.', 
          status: 'pending' 
        },
        { 
          slideId: 'slide6', 
          title: 'Getting Started', 
          content: 'Contact our sales team for a personalized demo and see how our solution can transform your organization\'s collaboration.', 
          status: 'pending' 
        },
      ];
  ...
  const startNarrationGeneration = async () => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to start narration generation';
      setNarrationState({
        status: 'error',
        progress: 0,
        currentSlide: 0,
  ...
  const loadNarrationResults = async () => {
    ...
    try {
      ...
      if (manifestResponse.success && manifestResponse.data) {
        ...
        const completeAudioUrl = manifestResponse.data.combined_audio_url || manifestResponse.data.audio?.combined_output_path;
        if (completeAudioUrl) {
              ...
              {narrationState.status !== 'idle' && narrationState.status !== 'error' && (
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between text-xs text-muted-foreground">
                    <span>Slide {narrationState.currentSlide} of {narrationState.totalSlides}</span>
                ...
                {slides.map((slide, index) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      <Badge variant={
                        slide.status === 'completed' ? 'default' :
                        slide.status === 'generating' ? 'secondary' :
                        slide.status === 'error' ? 'destructive' : 'outline'
                      }>
                        {slide.status}
                      </Badge>
                      {slide.audioUrl && (
        ...
        <TabsContent value="script">
          <ScriptEditor 
            slides={slides.map(slide => ({
              ...
              duration: slide.duration || Math.max(5, Math.round((slide.content.split(/\s+/).length / 160) * 60)),
            ...
            onScriptUpdate={(slideId, newScript) => {
              setSlides(prev => prev.map(slide => 
                slide.slideId === slideId 
                  ? { ...slide, generatedNarration: newScript }
                  : slide
              ));
            }}
          />
